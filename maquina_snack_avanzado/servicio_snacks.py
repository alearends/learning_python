...
from maquina_snack_avanzado.snack import Snack
...
class ServicioSnacks:
    ...
    def __init__(self):
        self.snacks = []
        # Revisar si ya existe el archivo snacks.txt
        ...
        try:
            with open(self.NOMBRE_ARCHIVO, 'r') as archivo:
                for linea in archivo:
                    ...
                    snack = Snack(nombre, float(precio), int(id_snack))
                    snacks.append(snack)
            # Asegurar que el contador continúe desde el último ID
            if snacks:
                Snack.contador_snacks = max(snack.id_snack for snack in snacks)
        except Exception as e:
